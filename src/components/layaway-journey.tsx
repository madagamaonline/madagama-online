...
export function LayawayJourney({ activeStep = 0 }: { activeStep?: number }) {
  return <div className="relative grid grid-cols-4" aria-label="Layaway lifecycle">
    <div className="absolute left-[12.5%] right-[12.5%] top-4 h-px bg-border" aria-hidden="true"><span className="block h-full bg-primary transition-[width] duration-500 ease-[var(--ease-enter)]" style={{ width: `${Math.max(0, Math.min(100, activeStep / 3 * 100))}%` }}/></div>
    {steps.map((step, index) => <div key={step.label} className="relative z-10 flex flex-col items-center text-center">
      <span className={cn("motion-interactive flex h-8 w-8 items-center justify-center rounded-lg border bg-surface shadow-sm", index <= activeStep ? "border-primary bg-primary text-primary-foreground" : "border-border text-faint", index === activeStep && "motion-check-in ring-4 ring-primary-soft")}><step.icon className="h-4 w-4"/></span>
