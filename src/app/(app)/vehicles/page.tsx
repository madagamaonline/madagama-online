...
import { Plus, Tractor, Warehouse, BadgeDollarSign } from "lucide-react";
...
export default async function VehiclesPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; kind?: string }> }) {
  ...
  return <div>
    ...
    <Card><CardContent className="p-0"><form className="flex flex-col gap-2 border-b border-border p-4 sm:flex-row"><input type="hidden" name="q" value={q} /><ListSearch placeholder="Search make, model, engine, chassis or supplier…" className="relative min-w-0 flex-1" /><Select name="kind" defaultValue={kind ?? ""} className="sm:w-48"><option value="">All vehicle types</option>{kinds.map((k) => <option key={k} value={k}>{labels[k]}</option>)}</Select><Select name="status" defaultValue={status ? status[0] + status.slice(1).toLowerCase() : ""} className="sm:w-40"><option value="">All statuses</option>{statuses.map((s) => <option key={s}>{s[0] + s.slice(1).toLowerCase()}</option>)}</Select><Button type="submit" variant="outline">Filter</Button></form>
