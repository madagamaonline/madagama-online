...
export function AddAdvance({ employees }: { employees: { id: string; name: string }[] }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Pay Advance</CardTitle>
      </CardHeader>
      <CardContent>
        <form ref={ref} action={action} className="space-y-3">
          {state.error && <div className="rounded-lg bg-danger-soft px-3 py-2 text-sm text-danger-ink">{state.error}</div>}
          {state.ok && <div className="rounded-lg bg-primary-soft px-3 py-2 text-sm text-primary-ink">Advance recorded. It will be recovered from the next payroll.</div>}
          <div>
          ...
          <div>
            ...
            <Input id="note" name="note" placeholder="e.g. Festival advance" />
          </div>
          <Button type="submit" className="w-full" disabled={pending}>
            <Plus className="h-4 w-4" /> {pending ? "Saving…" : "Pay Advance"}
          </Button>
        </form>
      </CardContent>
    </Card>
