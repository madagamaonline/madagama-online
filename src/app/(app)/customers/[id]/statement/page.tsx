  ...
  const [customer, setting] = await Promise.all([
    prisma.customer.findUnique({
      ...
      include: {
        ...
        layaways: { orderBy: { createdAt: "asc" }, include: { payments: { orderBy: { paidDate: "asc" } } } },
      },
    }),
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="print-area rounded-xl border border-border bg-surface p-8 shadow-sm">
        ...
        <h3 className="mb-2 mt-6 text-sm font-semibold">Layaway history (not credit)</h3>
        {customer.layaways.length === 0 ? <p className="text-sm text-muted">No layaways on record.</p> : <table className="w-full text-sm"><thead><tr className="border-y border-border text-left text-muted"><th className="py-2">Order</th><th>Status</th><th className="text-right">Total</th><th className="text-right">Collected</th><th className="text-right">Unpaid</th></tr></thead><tbody>{customer.layaways.map((order)=><tr key={order.id} className="border-b border-border"><td className="py-2 font-mono">LAY-{String(order.orderNumber).padStart(6,"0")}</td><td>{order.status.replaceAll("_"," ")}</td><td className="text-right">{formatLKR(order.total)}</td><td className="text-right">{formatLKR(order.collectedAmount)}</td><td className="text-right">{formatLKR(Math.max(0,toNum(order.total)-toNum(order.collectedAmount)))}</td></tr>)}</tbody></table>}
