...
const summary = [
  { status: "COLLECTED" as const, label: "Waiting to send", icon: Clock3, className: "text-clay-ink bg-clay-soft" },
  { status: "NEEDS_ATTENTION" as const, label: "Needs attention", icon: AlertTriangle, className: "text-danger-ink bg-danger-soft" },
  { status: "MCASH_SENT" as const, label: "Sent through mCash", icon: Send, className: "text-success-ink bg-success-soft" },
];

// Legacy tile: only receipts finished under the removed LOLC confirmation checkpoint sit here.
const legacyConfirmedTile = { status: "LOLC_CONFIRMED" as const, label: "Confirmed", icon: CheckCircle2, className: "text-primary-ink bg-primary-soft" };
...
export default async function LolcReceiptPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  const tiles = (countMap.get("LOLC_CONFIRMED") ?? 0) > 0 ? [...summary, legacyConfirmedTile] : summary;

  return <div className="mx-auto max-w-7xl">
    <PageHeader title="LOLC Receipts" subtitle="Track customer collections and mCash remittance." action={
